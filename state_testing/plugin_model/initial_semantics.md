# GlancesPluginModel – Initial semantics (fixed)

Component: `glances/plugins/plugin/model.py` – class `GlancesPluginModel` (view/history/alert only; no `update()`).
...
- **Preconditions:** `history_enable()` is true; `_limits['history_size']` exists (otherwise KeyError). For dict stats, each `i['name']` from `get_items_history_list()` must be a key in `get_export()` (else KeyError).
