...
3. **SUT environment:** The fuzzer imports `GlancesHistory` directly from `glances.history`. Activate the SUT environment (see state_testing README) so that Glances and its dependencies are available. The full package init may create cache dirs (e.g. under `~/.local`); run in a writable environment.
